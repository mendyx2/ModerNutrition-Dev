...
export default function InviteModal({ isOpen, onClose, user }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-md w-full border border-forest-subtle shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-forest-dark text-white p-5 flex items-center justify-between border-b-2 border-gold">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-extrabold font-heading text-white">Invite & Sponsor Members</h3>
              <p className="text-xs text-gray-300">Grow your binary team organization</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-5 bg-surface">
          ...
          {/* Member Code Badge */}
          <div className="bg-white p-4 rounded-xl border border-forest-subtle flex items-center justify-between">
            <div>
              <span className="text-[11px] font-bold text-muted uppercase tracking-wider block">Your Sponsor Code</span>
              <strong className="text-lg font-mono font-extrabold text-forest-dark">{memberCode}</strong>
            </div>
            <div className="px-3 py-1 bg-gold/10 border border-gold/30 rounded-lg text-gold-dark text-xs font-bold">
              Active Sponsor
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                readOnly
                value={referralLink}
                className="w-full px-3.5 py-2.5 bg-white rounded-xl border border-gray-300 font-mono text-xs text-forest-dark font-bold focus:ring-2 focus:ring-forest outline-none"
              />
              <button
                onClick={handleCopy}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <button
                ...
                className="flex items-center justify-center space-x-2 py-2.5 px-3 rounded-xl bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-800 text-xs font-bold transition-colors"
              >
              ...
              <button
                ...
                className="flex items-center justify-center space-x-2 py-2.5 px-3 rounded-xl bg-blue-50 hover:bg-blue-100 border border-blue-200 text-blue-800 text-xs font-bold transition-colors"
              >
